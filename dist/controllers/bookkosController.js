...
exports.printBookingPDF = exports.deleteBooking = exports.updateBooking = exports.getBookingById = exports.getAllBookings = exports.updateBookingStatus = exports.getOwnerBookings = exports.getUserBookings = exports.createBooking = void 0;
const client_1 = require("@prisma/client");
const pdf_lib_1 = require("pdf-lib"); // gunakan pdf-lib untuk hasil rapi
...
const createBooking = async (req, res) => {
    try {
        ...
        let diffmonth = (end.getFullYear() - start.getFullYear()) * 12 +
            (end.getMonth() - start.getMonth());
        // Kalau ada sisa hari lebih dari 0, hitung 1 bulan tambahan
        if (end.getDate() > start.getDate())
            diffmonth++;
        if (diffmonth <= 0)
            diffmonth = 1; // minimal 1 bulan
...
const printBookingPDF = async (req, res) => {
    try {
        const { bookingId } = req.params;
        const booking = await prisma.book.findUnique({
            where: { id: Number(bookingId) },
        ...
        const teal = (0, pdf_lib_1.rgb)(0.1, 0.5, 0.5);
        const gray = (0, pdf_lib_1.rgb)(0.6, 0.6, 0.6);
        const drawText = (text, x, y, size = 12, color = textColor) => {
        ...
        drawText("BUKTI PEMESANAN KAMAR", 160, height - 50, 18, (0, pdf_lib_1.rgb)(1, 1, 1));
        // ================== INFO BOOKING ==================
        let y = height - 120;
        ...
        y -= 30;
        drawText(`Nama Penyewa     : ${booking.user.name}`, 80, y);
        y -= 22;
        drawText(`Email            : ${booking.user.email}`, 80, y);
        y -= 22;
        drawText(`Nama Kos         : ${booking.kamarKos.kos.name}`, 80, y);
        y -= 22;
        drawText(`Alamat Kos       : ${booking.kamarKos.kos.address ?? "-"}`, 80, y);
        y -= 22;
        drawText(`Kamar            : ${booking.kamarKos.name}`, 80, y);
        y -= 22;
        drawText(`Tanggal Mulai    : ${new Date(booking.startDate).toLocaleDateString("id-ID")}`, 80, y);
        y -= 22;
        drawText(`Tanggal Selesai  : ${booking.endDate ? new Date(booking.endDate).toLocaleDateString("id-ID") : "-"}`, 80, y);
        y -= 22;
        drawText(`Total Harga      : Rp ${(booking.totalPrice ?? 0).toLocaleString("id-ID")}`, 80, y);
        y -= 22;
        drawText(`Status           : ${booking.status.toUpperCase()}`, 80, y);
        y -= 40;
        // ================== FOOTER ==================
        page.drawLine({
            start: { x: 80, y },
        ...
        y -= 30;
        drawText("Terima kasih telah melakukan booking di aplikasi NGKOST.", 80, y, 11, (0, pdf_lib_1.rgb)(0.2, 0.2, 0.2));
...
exports.printBookingPDF = printBookingPDF;
